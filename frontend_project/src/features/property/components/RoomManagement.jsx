        ...
        <table className="w-full text-left border-collapse min-w-[1400px]">
          <thead>
            <tr className="bg-[#f8fafc] dark:bg-slate-800/50 text-[#64748b] dark:text-slate-400 text-[11px] font-bold uppercase tracking-wider border-b border-slate-200 dark:border-slate-800">
              ...
              <th className="px-6 py-4 border-r border-slate-200 dark:border-slate-800 w-40">Building Name</th>
          ...
          <tbody className="text-[13px] divide-y divide-slate-100 dark:divide-slate-800">
            {rooms.map((room) => (
              <tr key={room.id} className="hover:bg-emerald-50/40 dark:hover:bg-emerald-500/5 transition-all h-14 group">
                ...
                <td className="px-6 py-2 border-r border-slate-100 dark:border-slate-800 font-medium text-slate-500 dark:text-slate-500 font-mono text-xs">{room.shortName}</td>
                <td className="px-6 py-2 border-r border-slate-100 dark:border-slate-800 text-slate-600 dark:text-slate-400">{room.roomTypeName}</td>
                <td className="px-6 py-2 border-r border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-500">{room.buildingName}</td>
                <td className="px-6 py-2 border-r border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-500">{room.floorName}</td>
                ...
                <td className="px-4 py-2 text-center border-r border-slate-100 dark:border-slate-800 bg-red-50/10">
                  <div className="flex justify-center transition-transform hover:scale-110">
                    {room.isNonRoom ? <Ban className="w-4.5 h-4.5 text-red-500 drop-shadow-sm" /> : <Minus className="w-3 h-3 text-slate-300 dark:text-slate-700 opacity-30" />}
                  </div>
                </td>
