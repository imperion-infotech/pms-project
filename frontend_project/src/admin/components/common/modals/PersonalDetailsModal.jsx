      ...
      <div className="bg-white dark:bg-slate-900 rounded-[40px] shadow-2xl w-full max-w-4xl relative z-10 overflow-hidden flex flex-col md:flex-row max-h-[90vh] animate-in zoom-in-95">
        ...
        <div className="w-full md:w-1/3 bg-slate-50 dark:bg-slate-800/50 p-8 flex flex-col items-center border-r border-slate-100 dark:border-slate-800 shrink-0">
          <div className="text-center mb-8">
            <h3 className="font-black text-emerald-500 uppercase tracking-tighter">New Profile</h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Capture guest assets</p>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900/50">
            <div><h2 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Personal Details</h2><p className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Industrial Mapping</p></div>
          ...
          <form onSubmit={handleSubmit} className="p-8 overflow-y-auto custom-scrollbar space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input required type="text" value={formData.firstName} onChange={(e) => setFormData({ ...formData, firstName: e.target.value })} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 rounded-2xl text-sm focus:ring-2 focus:ring-emerald-500/20 outline-none" />
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Last Name</label>
                <input required type="text" value={formData.lastName} onChange={(e) => setFormData({ ...formData, lastName: e.target.value })} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 rounded-2xl text-sm focus:ring-2 focus:ring-emerald-500/20 outline-none" />
              </div>
              <div className="col-span-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Company Name</label>
                <div className="relative"><Building size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" /><input type="text" value={formData.companyName} onChange={(e) => setFormData({ ...formData, companyName: e.target.value })} className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-800 rounded-2xl text-sm focus:ring-2 focus:ring-emerald-500/20 outline-none" /></div>
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <textarea required rows="2" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 rounded-2xl text-sm focus:ring-2 focus:ring-emerald-500/20 outline-none resize-none"></textarea>
              </div>
            </div>
