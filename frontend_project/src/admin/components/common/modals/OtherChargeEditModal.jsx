...
import { Zap, X, Save, CheckCircle2 } from 'lucide-react'

export const OtherChargeEditModal = ({
  isOpen,
  setIsOpen,
  editOtherCharge,
  setEditOtherCharge,
  ...
  const toggleField = (field) => {
    setEditOtherCharge((prev) => ({ ...prev, [field]: !prev[field] }))
  }
      ...
      <div className="dark:bg-surface-100 animate-in zoom-in-95 relative z-10 w-full max-w-2xl overflow-hidden rounded-3xl border border-slate-200/50 bg-white shadow-2xl duration-300 dark:border-slate-800/50">
        ...
        <form onSubmit={onSubmit} className="scrollbar-hide max-h-[75vh] overflow-y-auto p-8">
          <div className="grid grid-cols-2 gap-6">
            <div className="col-span-1">
              ...
              <input
                required
                type="text"
                value={editOtherCharge.otherChargeShortName}
                onChange={(e) =>
                  setEditOtherCharge({ ...editOtherCharge, otherChargeShortName: e.target.value })
                }
            ...
            <div className="col-span-1">
              ...
              <select
                value={editOtherCharge.categoryName}
                onChange={(e) =>
                  setEditOtherCharge({ ...editOtherCharge, categoryName: e.target.value })
                }
            ...
            <div className="col-span-2">
              ...
              <input
                required
                type="text"
                value={editOtherCharge.otherChargeName}
                onChange={(e) =>
                  setEditOtherCharge({ ...editOtherCharge, otherChargeName: e.target.value })
                }
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-800 transition-all focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 focus:outline-none dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-200"
              />
            </div>
              ...
              {[
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`rounded-lg p-2 transition-colors ${editOtherCharge[toggle.field] ? 'bg-orange-500/10 text-orange-600' : 'bg-slate-200 text-slate-400 dark:bg-slate-700'}`}
                    >
                  ...
                  <label className="relative inline-flex cursor-pointer items-center">
                    <input
                      ...
                      checked={editOtherCharge[toggle.field]}
            ...
            {editOtherCharge.reoccureCharge && (
              <div className="col-span-2">
                ...
                <input
                  type="number"
                  value={editOtherCharge.reoccureChargeFrequency}
                  onChange={(e) =>
                    setEditOtherCharge({
                      ...editOtherCharge,
          ...
          <div className="flex gap-4 pt-8">
            ...
            <button
              type="submit"
              disabled={!editOtherCharge.otherChargeName}
              className="flex flex-2 items-center justify-center gap-2 rounded-2xl bg-orange-600 py-3.5 text-xs font-extrabold tracking-widest text-white shadow-xl shadow-orange-600/30 transition-all hover:bg-orange-700 active:scale-95 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none"
            >
              <Save className="h-4 w-4" />
              <span>UPDATE CHARGE TYPE</span>
            </button>
          </div>
        </form>
      </div>
