...
import { CreditCard, X, Save, CheckCircle2 } from 'lucide-react'

export const PaymentTypeEditModal = ({
  isOpen,
  setIsOpen,
  editPaymentType,
  setEditPaymentType,
        ...
        <form onSubmit={onSubmit} className="space-y-6 p-8">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-1">
              ...
              <input
                required
                type="text"
                value={editPaymentType.paymentTypeShortName}
                onChange={(e) =>
                  setEditPaymentType({ ...editPaymentType, paymentTypeShortName: e.target.value })
                }
            ...
            <div className="col-span-1">
              ...
              <select
                value={editPaymentType.categoryName}
                onChange={(e) =>
                  setEditPaymentType({ ...editPaymentType, categoryName: e.target.value })
                }
          ...
          <div>
            ...
            <input
              required
              type="text"
              value={editPaymentType.paymentTypeName}
              onChange={(e) =>
                setEditPaymentType({ ...editPaymentType, paymentTypeName: e.target.value })
              }
              className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-800 transition-all focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 focus:outline-none dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-200"
            />
          </div>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/30">
            <div className="flex items-center gap-3">
              <div
                className={`rounded-lg p-2 transition-colors ${editPaymentType.creditCardProcessing ? 'bg-emerald-500/10 text-emerald-600' : 'bg-slate-200 text-slate-400 dark:bg-slate-700'}`}
              >
            ...
            <label className="relative inline-flex cursor-pointer items-center">
              <input
                ...
                checked={editPaymentType.creditCardProcessing}
                onChange={(e) =>
                  setEditPaymentType({ ...editPaymentType, creditCardProcessing: e.target.checked })
                }
              />
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              type="submit"
              disabled={!editPaymentType.paymentTypeName || !editPaymentType.paymentTypeShortName}
              className={`flex flex-2 items-center justify-center gap-2 rounded-2xl py-3.5 text-xs font-extrabold tracking-widest text-white shadow-xl transition-all active:scale-95 ${!editPaymentType.paymentTypeName || !editPaymentType.paymentTypeShortName ? 'cursor-not-allowed bg-slate-300 shadow-none grayscale dark:bg-slate-800' : 'bg-orange-600 shadow-orange-600/30 hover:bg-orange-700'}`}
            >
