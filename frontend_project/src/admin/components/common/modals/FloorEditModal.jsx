...
export const FloorEditModal = ({ isOpen, setIsOpen, editFloor, setEditFloor, handleUpdateFloor, floors = [] }) => {
  ...
  const isDuplicate = floors.some(f => String(f.name).toLowerCase() === String(editFloor.name).toLowerCase() && f.id !== editFloor.id);
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-sm dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-inner resize-none"
            ></textarea>
          </div>
