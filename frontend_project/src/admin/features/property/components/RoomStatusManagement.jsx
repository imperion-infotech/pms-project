...
import { PlusCircle, Pencil, Trash2, AlertTriangle, X, CheckSquare } from 'lucide-react'
...
const RoomStatusManagement = ({
  ...
  const {
    ...
    handleConfirmDelete,
    handleCancelDelete,
  } = useRoomStatusController({ roomStatuses, onDelete, currentPage, itemsPerPage })
                  ...
                  <td colSpan="6" className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center justify-center gap-3">
                      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-50 dark:bg-slate-800/20">
                        <CheckSquare className="h-8 w-8 text-slate-200 dark:text-slate-700" />
                      </div>
                      <p className="text-sm font-medium text-slate-400">No statuses defined</p>
                      <button
                        onClick={() => setIsRoomStatusModalOpen(true)}
        ...
        <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
          ...
          <div className="animate-in zoom-in-95 relative z-10 w-full max-w-sm overflow-hidden rounded-3xl border border-red-100 bg-white p-6 shadow-2xl transition-all dark:border-red-900/30 dark:bg-slate-900">
            <div className="mb-4 flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-800 dark:text-white">Delete Status</h3>
