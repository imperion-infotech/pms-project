...
import useBuildingController from '../controllers/useBuildingController';

/**
 * View: BuildingManagement
 * Professional UI for managing buildings, integrated with MVC controller.
 */
    ...
    <div className="bg-white dark:bg-surface-100 rounded-xl shadow-md border border-slate-200 dark:border-slate-800 transition-colors duration-300">
      ...
      <div className="p-4 sm:p-6 flex flex-col sm:flex-row justify-between items-center border-b border-slate-100 dark:border-slate-800 gap-4">
        <div className="text-center sm:text-left">
          ...
          <p className="text-xs md:text-sm text-slate-400 font-medium">Manage and organize physical property buildings</p>
        </div>
        ...
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead className="sticky top-0 z-10">
            <tr className="bg-[#f8fafc] dark:bg-slate-800 text-[#64748b] dark:text-slate-400 text-[11px] font-bold uppercase tracking-wider border-b border-slate-200 dark:border-slate-800">
              ...
              <th className="px-8 py-4 border-r border-slate-200 dark:border-slate-800">Building Name</th>
                ...
                <td colSpan="4" className="px-8 py-20 text-center text-slate-400 dark:text-slate-500">
                  <div className="flex flex-col items-center justify-center space-y-2">
                    ...
                    <p className="font-medium uppercase tracking-widest text-[10px]">
                      {searchTerm ? 'No matching buildings' : 'No buildings defined'}
                    </p>
                  </div>
                </td>
              </tr>
            ) : (
              processedBuildings.map((building, idx) => (
                <tr key={building.id} className="group hover:bg-emerald-50/40 dark:hover:bg-emerald-500/5 transition-all h-14">
                  ...
                  <td className="px-8 py-2 font-bold text-slate-700 dark:text-slate-300 border-r border-slate-100 dark:border-slate-800 uppercase tracking-tight">{building.name}</td>
                  <td className="px-8 py-2 text-slate-500 dark:text-slate-400 border-r border-slate-100 dark:border-slate-800">{building.description || '—'}</td>
                  <td className="px-8 py-2">
                    <div className="flex justify-center gap-3">
                      <button
                        onClick={() => onEdit(building)}
                        className="p-1.5 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg text-blue-500 transition-colors" title="Edit Building"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleDeleteClick(building)}
                        className="p-1.5 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg text-red-500 transition-colors" title="Delete Building"
                      >
