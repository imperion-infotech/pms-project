...
import { PlusCircle, Pencil, Trash2, AlertTriangle, X, Receipt, CheckCircle2, XCircle } from 'lucide-react'
...
const TaxManagement = ({
  ...
  const {
    ...
    handleConfirmDelete,
    handleCancelDelete,
  } = useTaxController({ taxes, onDelete, currentPage, itemsPerPage })
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {processedTaxes.length === 0 ? (
                  ...
                  <td colSpan="6" className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center justify-center gap-3">
                      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-50 dark:bg-slate-800/20">
                        <Receipt className="h-8 w-8 text-slate-200 dark:text-slate-700" />
                      </div>
                      <p className="text-sm font-medium text-slate-400">No tax masters defined</p>
                      <button onClick={() => setIsTaxModalOpen(true)} className="text-xs font-bold text-emerald-500 hover:underline">
                        Add your first tax master
                      </button>
                    </div>
                  </td>
                </tr>
                ...
                processedTaxes.map((tax, idx) => (
                  <tr key={tax.id} className="group transition-all hover:bg-emerald-500/5 dark:hover:bg-emerald-500/10">
                    <td className="px-6 py-5 text-center font-mono text-xs font-bold text-slate-300 group-hover:text-emerald-500 dark:text-slate-600">{getIndex(idx)}</td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{tax.taxMasterName}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
        ...
        <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
          ...
          <div className="animate-in zoom-in-95 relative z-10 w-full max-w-sm overflow-hidden rounded-3xl border border-red-100 bg-white p-6 shadow-2xl transition-all dark:border-red-900/30 dark:bg-slate-900">
            <div className="mb-4 flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-800 dark:text-white">Delete Tax Master</h3>
