// Property feature components - TaxManagement
/**
 * TaxManagement.jsx (Admin Domain -> Features: Property)
 * 
 * Feature component to manage Tax Masters. 
 * Contains tables and UI logic strictly constrained to creating, reading, updating, deleting (CRUD) Tax Master records.
 */
import React, { useState } from 'react';
import { PlusCircle, Pencil, Trash2, AlertTriangle, X, Receipt } from 'lucide-react';
  ...
  return (
    <div className="bg-white dark:bg-[#1e293b] rounded-xl shadow-md border border-slate-200 dark:border-slate-800 overflow-hidden transition-colors duration-300">
      ...
      <div className="p-6 flex flex-col lg:flex-row justify-between items-center border-b border-slate-100 dark:border-slate-800 gap-4">
        <div>
          <h2 className="text-xl font-bold text-[#1a2b4b] dark:text-slate-100 font-heading tracking-tight flex items-center gap-2">
            <Receipt className="w-6 h-6 text-emerald-500" />
            Tax Management
          </h2>
          <p className="text-sm text-slate-400 font-medium">Configure and manage tax masters for property services</p>
        </div>
        <button
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-[#f8fafc] dark:bg-slate-800/50 text-[#64748b] dark:text-slate-400 text-[11px] font-bold uppercase tracking-wider border-b border-slate-200 dark:border-slate-800">
              ...
              <th className="px-6 py-4 border-r border-slate-200 dark:border-slate-800">Tax Name</th>
              <th className="px-6 py-4 border-r border-slate-200 dark:border-slate-800">Tax Type</th>
              <th className="px-6 py-4 border-r border-slate-200 dark:border-slate-800 text-center">Per Day</th>
              <th className="px-6 py-4 border-r border-slate-200 dark:border-slate-800 text-center">Per Stay</th>
              <th className="px-6 py-4 border-r border-slate-200 dark:border-slate-800">Created On</th>
          ...
          <tbody className="text-[13px] divide-y divide-slate-100 dark:divide-slate-800">
            {taxes.length === 0 ? (
              <tr>
                <td colSpan="7" className="px-8 py-10 text-center text-slate-400 dark:text-slate-500 text-sm italic">
                  No tax masters found. Please add your first tax configuration.
                </td>
              </tr>
            ) : (
              taxes.map((tax, idx) => (
                <tr key={tax.id ?? idx} className="hover:bg-emerald-50/40 dark:hover:bg-emerald-500/5 transition-all h-16 group">
                  {/* ID */}
                  ...
                  <td className="px-6 py-3 border-r border-slate-100 dark:border-slate-800 font-bold text-slate-600 dark:text-slate-400">
                    <span className="bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-[10px] uppercase">
                      {tax.taxTypeEnum?.replace('_', ' ') || '—'}
                    </span>
                  </td>
                  {/* Per Day */}
