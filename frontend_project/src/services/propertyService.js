...
export const propertyService = {
  ...
  updateDocumentType: (id, data) => handleResponse(() => api.put(`/admin/updatedocumenttype/${id}`, data)),
  deleteDocumentType: (id) => handleResponse(() => api.delete(`/admin/deletedocumenttype/${id}`)),
  searchDocumentTypes: (query) => handleResponse(() => api.get('/user/documenttype/search', { params: { query } })),

  // --- IMAGE MANAGEMENT ---
  uploadImage: (formData) => handleResponse(() => api.post('/user/upload', formData, {headers: { 'Content-Type': 'multipart/form-data' }, }),
